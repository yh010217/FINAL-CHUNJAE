...
//item.content.substring(item.content.length - 3) === "svg"
    ...
    const answerExplain = function (){
        ...
        fetch('/convertImage', {
            ...
            body: JSON.stringify(answerExplainList)
        }).then(response => {
            if (!response.ok)
                throw new Error('not load');
            return response.json();
            ...
            dataList.map(item => {
                ...
                if (index === 0) {
                    ...
                    const title_txt = document.createTextNode(saveName+"_정답 해설");
                    const gradeClass_txt = document.createTextNode("학년 반 번");
                    const name_txt = document.createTextNode("이름 :");
        ...
        fetch('/convertImage', {
            ...
            dataList.map(item => {
                ...
                if (index === 0) {
                    ...
                    const title_txt = document.createTextNode(saveName+"_문제 정답 해설");
                    const gradeClass_txt = document.createTextNode("학년 반 번");
                    const name_txt = document.createTextNode("이름 :");
